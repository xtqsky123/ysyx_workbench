...
static word_t csr_read(uint32_t csr) {
  switch (csr) {
    ...
    case CSR_MVENDORID: return 0x79737978u;
    case CSR_MARCHID:   return 100014131u;
    default: panic("unsupported csr read: 0x%x", csr);
  }
}

static void csr_write(uint32_t csr, word_t val) {
  switch (csr) {
    ...
    case CSR_MCYCLE:
      cpu.mcycle = (cpu.mcycle & 0xffffffff00000000ull) | (uint32_t)val;
      break;
    case CSR_MCYCLEH:
      cpu.mcycle = (cpu.mcycle & 0x00000000ffffffffull) | ((uint64_t)(uint32_t)val << 32);
      break;
...
enum {
  TYPE_I, TYPE_U, TYPE_S, TYPE_J, TYPE_B, TYPE_R, TYPE_SHI,
  TYPE_N, // none
};
...
#define immSHI() do { *imm = BITS(i, 24, 20); } while (0)
...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  switch (type) {
    ...
    case TYPE_SHI: src1R();        immSHI(); break;
    case TYPE_N: break;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , SHI, R(rd) = src1 << imm);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , SHI, R(rd) = src1 >> imm);
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , SHI, R(rd) = (word_t)((sword_t)src1 >> imm));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I,
      ...
      if (rd == 1) ftrace_call(s->pc, s->dnpc);
      else if (rd == 0 && BITS(s->isa.inst, 19, 15) == 1 && imm == 0) ftrace_ret(s->pc, s->dnpc));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (word_t)((sword_t)src1 >> BITS(src2, 4, 0)));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R,
      ...
      else if (src1 == 0x80000000u && src2 == 0xffffffffu) R(rd) = src1;
      else R(rd) = (word_t)((sword_t)src1 / (sword_t)src2));
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R,
      if (src2 == 0) R(rd) = (word_t)-1;
      else R(rd) = src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R,
      if (src2 == 0) R(rd) = src1;
      else if (src1 == 0x80000000u && src2 == 0xffffffffu) R(rd) = 0;
      else R(rd) = (word_t)((sword_t)src1 % (sword_t)src2));
